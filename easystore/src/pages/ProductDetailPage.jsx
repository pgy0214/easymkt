...
export default function ProductDetailPage({ productId }) {
  ...
  if (!product) return <p className="p-6 text-sm text-gray-400">불러오는 중...</p>
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      <button onClick={() => navigate('/')} className="mb-4 text-sm text-gray-500 hover:underline">
        ← 목록으로
      </button>
      ...
      <h1 className="mb-4 text-lg font-bold text-gray-900">{product.name}</h1>
      ...
      <div className="mb-6 rounded-card border border-gray-200 p-4">
        {activeOptions.length === 0 ? (
          ...
          <>
            <label className="mb-1 block text-xs text-gray-500">옵션선택</label>
            <select
              value={selectedOptionId}
              onChange={(e) => setSelectedOptionId(e.target.value)}
              className="mb-3 w-full rounded-btn border border-gray-300 px-3 py-2 text-sm"
            >
              {activeOptions.map((o) => (
      ...
      {tab === 'price' && (
        <div className="space-y-2">
          ...
          {activeOptions.map((o) => (
            <div key={o.id} className="flex justify-between rounded-btn border border-gray-200 px-3 py-2 text-sm">
              <span>{o.label}</span>
