...
export default function CheckoutPage({ productId, optionId }) {
  ...
  if (!option) return <p className="p-6 text-sm text-danger-text">선택한 옵션을 찾을 수 없습니다.</p>
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-6">
      <button onClick={() => navigate(`/product/${product.id}`)} className="mb-4 text-sm text-gray-500 hover:underline">
        ← 뒤로
      </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <p className="text-xs text-gray-400">
          결제는 계좌이체로만 진행됩니다. 아래 정보 입력 후 주문하시면, 다음 화면에서 입금계좌를
          안내해드립니다. 입금 확인 후 순차적으로 처리됩니다.
        </p>
        <div>
