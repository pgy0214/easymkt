...
class Reviewer(Base):
    ...
    marketing_consent_at = Column(DateTime, nullable=True)  # 포털 셀프 회원가입 시 마케팅(문자 등) 수신 동의 시각 — 선택
...
class ReviewAccount(Base):
    ...
    browserbase_context_id = Column(String, nullable=True)  # Browserbase Context id — 계정당 한 번만 생성해서 저장, 로그인상태 유지용
