...
const EMPTY = { url: '', cooldown_days: 30 }
    ...
    <div className="space-y-4">
      <Card className="space-y-4">
        <div className="flex max-w-md flex-col gap-3">
          ...
          <div>
            <label className="block text-xs text-gray-500">재작업 가능 주기 (일)</label>
            <Input
              type="number"
              min="1"
      ...
      <div className="overflow-x-auto rounded-card border border-gray-200 bg-white">
          ...
          <tbody className="divide-y divide-gray-100">
            {stores.map((store) => (
              <tr key={store.id}>
                ...
                <td className="px-3 py-2 text-gray-700" title={store.address || ''}>
                  {store.address || '-'}
                </td>
                <td className="px-3 py-2">{store.cooldown_days}</td>
                <td className="px-3 py-2 text-gray-500">{formatDateTime(store.created_at)}</td>
                <td className="px-3 py-2">
        ...
        {stores.length === 0 && !loading && (
          <p className="p-3 text-sm text-gray-400">등록된 매장이 없습니다</p>
        )}
      </div>
